...
def setup_database():
    ...
    try:
        # Change directory to dataverse_backend to ensure imports work
        backend_path = Path(__file__).parent / "dataverse_backend"
        if not backend_path.exists():
            backend_path = Path.cwd() / "dataverse_backend"
...
def print_next_steps():
    ...
    print('   curl -X POST http://localhost:8000/api/upload -F "file=@retail_mart_processed_v1.csv"\n')
