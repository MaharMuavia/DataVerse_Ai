...
echo Location: %ROOT_DIR%\dataverse_backend
echo Port: 8001
echo.
...
start "DataVerse Backend" cmd /k "cd /d ""%ROOT_DIR%\dataverse_backend"" && python -m uvicorn app.main:app --host 127.0.0.1 --port 8001 --log-level info"
...
echo Location: %ROOT_DIR%\docs\assets\dashboard.html
echo.

REM Open the dashboard HTML file in default browser
start "" "%ROOT_DIR%\docs\assets\dashboard.html"
...
echo.
echo   📖 Full docs:   See docs\IMPLEMENTATION_GUIDE.md
echo   🧪 Test suite:  Run: python scripts\demo_client.py
echo.
