...
DEMO_DATASET = "retail_mart_processed_v1.csv"  # Retail dataset for demo
...
def test_upload():
    ...
    if not Path(DEMO_DATASET).exists():
        print(f"  ERROR: {DEMO_DATASET} not found")
    ...
    try:
        with open(DEMO_DATASET, "rb") as f:
            files = {"file": (DEMO_DATASET, f, "text/csv")}
            print(f"  Uploading {DEMO_DATASET}...")
...
def main():
    ...
    if not health_ok:
        ...
        print("  Start the server with: python -m uvicorn dataverse_backend.app.main:app --host 127.0.0.1 --port 8000")
        return
