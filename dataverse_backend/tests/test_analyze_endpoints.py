...
def test_ai_khata_report_analysis_uses_extracted_business_summary():
    ...
    assert report["dataset_profile"]["report_summary"]["Total Sales"] == "5000"
    assert report["column_roles"]["Amount (Rs)"] == "sales_amount"
    assert "Total Sales: 5000" in report["executive_summary"]
    assert "Udhaar Outstanding: 300" in report["executive_summary"]
